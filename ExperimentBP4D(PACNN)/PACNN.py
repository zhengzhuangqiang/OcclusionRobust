...
class PGUnit(nn.Module):
    def __init__(self, in_channels):
        ...
        self.fc = nn.Sequential(*[
            nn.Linear(512*6*6, 64), nn.ReLU(inplace=True),
        ])
...
class PACNN(nn.Module):
    def __init__(self, num_AU):
        ...
        for i in range(24): self._modules['PG%02d' % i] = PGUnit(512)
        self.classifier = nn.Sequential(*[
            nn.ReLU(inplace=True),
            nn.Linear(64*24, 1024), nn.Dropout(0.5), nn.ReLU(inplace=True),
